...
Java_com_google_oboe_samples_soundboard_MainActivity_startEngine(
    ...
    SynthesisEngine *engine = new SynthesisEngine(jNumSignals);
...
Java_com_google_oboe_samples_soundboard_MainActivity_stopEngine(JNIEnv *env, jobject instance,
                                                                jlong jEngineHandle) {
    auto engine = reinterpret_cast<SynthesisEngine *>(jEngineHandle);
    if (engine) {
        engine->stop();
        delete engine;
    } else {
...
Java_com_google_oboe_samples_soundboard_NoteListener_noteOff(JNIEnv *env, jobject thiz,
    ...
    auto *engine = reinterpret_cast<SynthesisEngine *>(engine_handle);
    if (engine) {
